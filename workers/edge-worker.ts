  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/reviews' && request.method === 'POST') {
        ...
        // Intercept legacy node IDs generated by cached frontends and safely remap them
        if (newNodes && Array.isArray(newNodes)) {
          let idMap = {};
          newNodes.forEach(n => {
            if (n.id && n.id < 1000000) {
              const fixedId = Math.floor(Math.random() * 100000000) + 1000000;
              idMap[n.id] = fixedId;
              n.id = fixedId;
            }
          });
          newNodes.forEach(n => {
            if (n.parent_id && idMap[n.parent_id]) {
              n.parent_id = idMap[n.parent_id];
            }
          });
          if (review && review.node_id && idMap[review.node_id]) {
            review.node_id = idMap[review.node_id];
          }
        }
